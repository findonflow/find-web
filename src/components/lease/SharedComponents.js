...
export function DurationLegend({ lease }) {
    ...
    if (user.addr !== lease.address) {
        durationLegend = <div className="nameAvailable p-3"><Image src="/assets/img/Tick_circle.png" alt="Green circle with a tick" height={"24px"} className="me-2" />{lease.name} is taken and valid until: <span style={{ color: "green" }}>{epochToJsDate(lease.validUntil) + " at " + epochToJsTime(lease.validUntil)}</span></div>
        if (lease.validUntil < Math.floor((new Date()).getTime() / 1000) && !lease.status === "") {
            durationLegend = <div className="nameLocked p-3"><Image src="/assets/img/Cross_circle.png" alt="Red circle with a cross" height={"24px"} className="me-2" />{lease.name} is taken and locked until: <span style={{ color: "red" }}>{epochToJsDate(lease.lockedUntil) + " at " + epochToJsTime(lease.lockedUntil)}</span></div>
        }
    }
    else{
        durationLegend = <Row>
